...
from mnema._query import _BASE, _DATA_SHA256, _DATA_URL
...
def test_a_substituted_snapshot_is_rejected_and_not_cached(tmp_path, monkeypatch):
    ...
    gz = tmp_path / "_data" / "mnema.db.gz"
    db = BaseDB("mnema-test", gz_path=gz, remote_url=_DATA_URL,
...
def test_a_matching_snapshot_is_accepted(tmp_path, monkeypatch):
    ...
    gz = tmp_path / "_data" / "mnema.db.gz"
    out = ensure_db("mnema-test", _DATA_URL, gz,
